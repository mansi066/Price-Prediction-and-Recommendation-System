...
async def root():
    return {
        "message": "Property Analytics API running",
        "frontend": "Next.js",
        "docs": "/docs"
    }
...
async def predict_price(features: PropertyFeatures):
    ...
    try:
        ...
        categorical_fields = {
            ...
            'agePossession': features.agePossession,
            'furnishing_type': features.furnishing_type,
            'luxury_category': features.luxury_category,
            'floor_category': features.floor_category
        }
        
        empty_fields = [field for field, value in categorical_fields.items() if not value or value.strip() == '']
        if empty_fields:
            raise HTTPException(
                status_code=400, 
                detail=f"Please select values for: {', '.join(empty_fields)}. These fields cannot be empty."
            )
